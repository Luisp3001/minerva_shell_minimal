...
// ScreenshotTool – wrapper that manages HyprQuickFrame lifecycle.
// Set active=true to launch, emits done() when finished.
QtObject {
    id: wrapper
    ...
    onActiveChanged: {
        if (active) {
            loader.sourceComponent = screenshotComponent;
        } else {
    ...
    property var screenshotComponent: Component {
        FreezeScreen {
            id: root
            ...
            function finish() {
                root.visible = false;
                // Don't set wrapper.active directly — it breaks the QML binding.
                // Emitting done() lets the parent set screenshotActive = false,
                // which flows through the binding to deactivate us.
                wrapper.done();
            }
            ...
            Component.onCompleted: {
                // Eagerly resolve activeScreen and hyprlandMonitor —
                // in standalone mode, onFocusedMonitorChanged fires at
                // startup, but when dynamically created the monitor is
                // already set and the signal won't re-emit.
                // We must snapshot these BEFORE the overlay takes focus.
            ...
            FileView {
                ...
                property string defaultPath: Qt.resolvedUrl(".").toString().replace(/^file:\/\//, "") + "theme.toml"
