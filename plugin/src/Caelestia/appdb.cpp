...
AppDb::AppDb(QObject* parent)
    ...
    // Ensure Qt can find the QSQLITE driver plugin even when the host process
    // (quickshell) is not linked against Qt6Sql and therefore does not add the
    // Qt plugin directory to the search path automatically.
    QCoreApplication::addLibraryPath("/usr/lib/qt6/plugins");

    if (!QSqlDatabase::isDriverAvailable("QSQLITE")) {
        qCWarning(lcAppDb) << "QSQLITE driver not available — AppDb will not persist frequencies";
    }

    auto db = QSqlDatabase::addDatabase("QSQLITE", m_uuid);
    db.setDatabaseName(":memory:");
    if (!db.open()) {
        qCWarning(lcAppDb) << "AppDb: failed to open in-memory DB:" << db.lastError().text();
    }
...
void AppDb::setPath(const QString& path) {
    ...
    if (!db.open()) {
        qCWarning(lcAppDb) << "AppDb: failed to open database" << newPath << "-" << db.lastError().text();
        return;
    }

    QSqlQuery query(db);
    if (!query.exec("CREATE TABLE IF NOT EXISTS frequencies (id TEXT PRIMARY KEY, frequency INTEGER)")) {
        qCWarning(lcAppDb) << "AppDb: failed to create frequencies table:" << query.lastError().text();
    }
